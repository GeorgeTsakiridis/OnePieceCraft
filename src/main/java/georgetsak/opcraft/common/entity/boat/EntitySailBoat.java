package georgetsak.opcraft.common.entity.boat;
...
public class EntitySailBoat extends Entity
{
    ...
    {
        if (entityIn instanceof EntitySailBoat)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.canPassengerSteer())
        {
            ...
            this.move(MoverType.SELF, this.motionX*1.2D, this.motionY*1.2D, this.motionZ*1.2D);
        }
        else
        {
            this.motionX = 0.0D;
        ...
        if (!list.isEmpty())
        {
            ...
            for (int j = 0; j < list.size(); ++j)
            {
                ...
                {
                    if (flag && this.getPassengers().size() < 4 && !entity.isRiding() && entity.width < this.width && entity instanceof EntityLivingBase && !(entity instanceof EntityWaterMob) && !(entity instanceof EntityPlayer))
                    {
    ...
    {
        Status EntitySailBoat$status = this.getUnderwaterStatus();

        if (EntitySailBoat$status != null)
        {
    ...
    @Nullable
    private EntitySailBoat.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
    ...
    public void updatePassenger(Entity passenger)
    {
        if (this.isPassenger(passenger))
        {
            ...
            switch (i){
                case 0: f += 2.5F; break;
                case 1: f += 0.2F; break;//0.6
                case 2: f += 2.5F; f2 += 0.8F; break;
                case 3: f += 0.2F; f2 += 0.8F; break;
            }
            f -= 0.3F;
            ...
            Vec3d vec3d = (new Vec3d((double)f, 0.0D, (double)f2)).rotateYaw(-this.rotationYaw * 0.017453292F - ((float)Math.PI / 2F));
    ...
    public boolean getSpawnParticles(){
        ...
        //TODO Particles spawn only when there is a passenger. Should also check for speed. (Non riding Players do not receive motion variables)
        return this.getControllingPassenger() != null; //&& (Math.abs(this.motionX) > 0.1D || Math.abs(this.motionZ) > 0.1D);
    }
