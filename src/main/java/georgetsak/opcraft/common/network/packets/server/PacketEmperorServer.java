...
public class PacketEmperorServer extends AbstractMessage.AbstractServerMessage<PacketEmperorServer>{
    ...
    private void emperor(EntityPlayer player, int level) {
        ...
        player.getServer().getEntityWorld().playSound(null, center, OPSoundEvent.emperor_haki, SoundCategory.NEUTRAL, (float)(10 + level*2) + 10f, 1f);
        ...
        if(!entities.isEmpty()){
            for(Entity entity : entities){
                if(entity instanceof EntityPlayer){
                    entity.attackEntityFrom(DamageSource.causePlayerDamage(player), MathUtils.calculateDamage((EntityPlayer)entity, 8F * level, true));
                }else{
